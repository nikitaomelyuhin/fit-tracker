...
export function describeWaterDebt(debt: WaterDebt | null): { text: string; tone: WaterDebtTone } {
  ...
  return {
    text: `По весам ушло на ${Math.abs(grams)} г больше, чем должно было по калориям — либо ты на самом деле тратишь больше, чем предполагает формула расхода (вес/рост/возраст × активность), либо ушла лишняя вода.`,
    tone: 'good',
  }
}
